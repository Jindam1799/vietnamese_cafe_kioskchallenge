/**
 * 진담카페 챌린지 v5.0 - 최종 완결 (크레딧 & 워터마크 & 모든 기능)
 */
...
const dialogueData = {
    ...
    "point_start": { text: "너 여기 카페 회원이야? 포인트 적립할 수 있대.", choices: [{ text: "응, 내 번호 입력할게~ 010-1234-5678", next: "go_keypad" }] },
...
function renderDialogue(key) {
    ...
    if(node.speaker === 'boss') chatHeaderTitle.innerText = "카페 사장님 👨‍🍳";
    setTimeout(() => {
...
function addChatMessage(side, text, isBoss = false) {
    ...
    let profileIcon = "👩"; if (isBoss) profileIcon = "👨‍🍳"; 
    msg.innerHTML = side === 'left' ? `<div class="friend-profile">${profileIcon}</div><div class="bubble">${text}</div>` : `<div class="bubble">${text}</div>`;
    log.appendChild(msg); log.scrollTop = log.scrollHeight;
}

// --- [3] 옵션 및 메뉴 ---
function openOptions(menu) {
    if (menu === "생강차") { showLayer('chat'); renderDialogue('ginger_1'); return; }
    if (menu === "생딸기우유") { showLayer('chat'); renderDialogue('berry_1'); return; }
    currentMenu = menu; document.getElementById('opt-menu-name').innerText = menu;
    const isCoffee = menu === "아메리카노"; const isSpec = ["초코케이크", "생딸기우유"].includes(menu);
    ...
    document.getElementById('option-sheet').classList.add('active'); updateOptionUI();
}

function switchTab(e, cat) { document.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active')); document.querySelectorAll('.menu-grid').forEach(g => g.classList.remove('active')); e.currentTarget.classList.add('active'); document.getElementById(cat + '-menu').classList.add('active'); }
function setOption(type, val) { currentOptions[type] = val; if (type === 'temp') currentOptions.ice = (val === 'HOT') ? "" : "보통"; updateOptionUI(); }
function updateOptionUI() { document.querySelectorAll('.pill').forEach(btn => { btn.classList.toggle('selected', currentOptions[btn.dataset.type] === btn.dataset.value); }); const isSpec = ["초코케이크", "소금빵", "생딸기우유"].includes(currentMenu); document.getElementById('ice-row').style.display = (currentOptions.temp === 'HOT' || isSpec) ? 'none' : 'block'; document.getElementById('shot-val').innerText = currentOptions.shot; }
...
function openCart() { cartListContainer.innerHTML = cart.length === 0 ? "<p style='padding:20px; color:#bbb; text-align:center;'>장바구니가 비어 있습니다.</p>" : ""; cart.forEach((i, idx) => { const div = document.createElement('div'); div.className = 'cart-item'; div.innerHTML = `<div><b style='color:#3d2b1f;'>${i.name}</b><br><small style='color:#888;'>${i.temp} ${i.ice} ${i.shot}샷</small></div><button class="cart-del-btn" onclick="removeFromCart(${idx})">삭제</button>`; cartListContainer.appendChild(div); }); cartDetailLayer.style.display = 'flex'; }
...
function showPointKeypad() { inputPhone = "010"; modalLayer.style.display = "flex"; renderKeypadContent(); }
function renderKeypadContent() { modalBox.innerHTML = `<div class="pop-in"><h3 style="margin-bottom:15px; font-weight:800;">포인트 적립</h3><div class="phone-display-container" id="phone-boxes">${renderPhoneBoxes(inputPhone)}</div><div class="keypad">${[1,2,3,4,5,6,7,8,9].map(n => `<button class="key" onclick="pressKey('${n}')">${n}</button>`).join('')}<button class="key" onclick="pressKey('C')">C</button><button class="key" onclick="pressKey('0')">0</button><button class="key enter" onclick="confirmPoints()">입력</button></div><button class="btn-confirm-large" style="background:#ddd; color:#666;" onclick="modalLayer.style.display='none'">취소</button></div>`; }
function renderPhoneBoxes(numStr) { const full = numStr.padEnd(11, " "); let html = ""; for (let i = 0; i < 11; i++) { html += `<div class="digit-box ${full[i] !== " " ? 'filled' : ''}">${full[i] === " " ? "" : full[i]}</div>`; if (i === 2 || i === 6) html += `<div class="phone-dash">-</div>`; } return html; }
...
function showFinalReceipt(method) {
    ...
    cart.forEach(i => { const p = prices[i.name] || 0; total += p; const isBev = ["아메리카노", "생강차", "생딸기우유"].includes(i.name); const detail = isBev ? `<br><small style="color:#888;">(${i.temp.toLowerCase()}, 샷 ${i.shot}개, 얼음 ${i.ice})</small>` : ""; itemsHtml += `<div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:12px;"><div style="text-align:left;"><strong>${i.name}</strong>${detail}</div><div>${p.toLocaleString()}원</div></div>`; });
    modalBox.innerHTML = `<div class="pop-in" style="text-align:center;"><h2 style="color:#2ecc71; margin-bottom:15px;">✔ 결제 완료</h2><div class="receipt"><p style="text-align:center; font-weight:900; margin-bottom:15px; padding-bottom:10px; border-bottom:2px solid #333;">JINDAM CAFE</p>${itemsHtml}<div style="border-top:1px dashed #aaa; margin-top:15px; padding-top:15px; display:flex; justify-content:space-between; font-weight:900; font-size:1.1rem;"><span>총액</span><span style="color:var(--accent);">${total.toLocaleString()}원</span></div></div><div style="background:#fdf2f0; padding:20px; border-radius:20px; margin-bottom:20px;"><span style="font-size:0.85rem; color:#e74c3c; font-weight:800;">대기 번호</span><div style="font-size:3rem; font-weight:900; color:#e74c3c;">124</div></div><button class="btn-confirm-large" onclick="location.reload()">처음으로</button></div>`;
}
function showCustomPopup(title, msg, btnText, cb) { modalLayer.style.display="flex"; modalBox.innerHTML=`<div class="pop-in"><h2 style="color:#ff4757;">${title}</h2><p style="margin:20px 0; line-height:1.5;">${msg}</p><button class="btn-confirm-large" id="c-btn">${btnText}</button></div>`; document.getElementById('c-btn').onclick=cb; layers.kiosk.classList.add('shake-ani'); setTimeout(() => layers.kiosk.classList.remove('shake-ani'), 400); }
